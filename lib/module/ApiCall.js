...
export async function setConfig(appConfig) {
  if (appConfig != null && appConfig instanceof AppConfig) {
    ...
    if (client_id != null && client_id !== 'undefined' && client_secret != null && client_secret !== 'undefined' && app_id != null && app_id !== 'undefined' && environment != null && environment !== 'undefined') {
      ...
      await EncryptedStorage.setItem('waivpay_sdk_config_app_id', JSON.stringify(appConfig));
    } else {
...
export async function getBalance(cardId) {
  ...
  return new Promise(async function (resolve) {
    const accessToken = await getAccessToken();
    const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.cards + '/' + cardId + EndPoints.balance;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, '');
...
export async function getTransactions(cardId) {
  ...
  return new Promise(async function (resolve) {
    const accessToken = await getAccessToken();
    const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.cards + '/' + cardId + EndPoints.transaction;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, '');
...
export async function getCardDetails(cardId) {
  ...
  return new Promise(async function (resolve) {
    const accessToken = await getAccessToken();
    const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.cards + '/' + cardId;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, '');
...
export async function createProfile(user) {
  ...
  return new Promise(async function (resolve) {
    const accessToken = await getAccessToken();
    const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.users;
...
export async function createOrder(order) {
  ...
  return new Promise(async function (resolve) {
    const accessToken = await getAccessToken();
    const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.orders;
...
export async function searchCards(mobile) {
  ...
  return new Promise(async function (resolve) {
    const accessToken = await getAccessToken();
    const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.cards + '?mobile_number=' + mobile;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, '');
...
export async function getProfile(userId) {
  ...
  return new Promise(async function (resolve) {
    const accessToken = await getAccessToken();
    const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.users + '/' + userId;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, '');
...
export async function updateProfile(user) {
  ...
  if (user.id != null) {
    return new Promise(async function (resolve) {
      const accessToken = await getAccessToken();
      const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.users + '/' + user.id;
...
export async function getOrders(user_id) {
  ...
  return new Promise(async function (resolve) {
    const accessToken = await getAccessToken();
    const url = getHostEndPoints(config) + EndPoints.appSpecific + config.app_id + EndPoints.orders + "?user_id=" + user_id;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, '');
...
//# sourceMappingURL=ApiCall.js.mapeText) {
...
export async function fileUpload(fileInput) {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    const url = getHostEndPointsCashback(config) + EndPointsCashBack.api + EndPointsCashBack.claims + EndPointsCashBack.fileUpload;
...
export async function listPromotions() {
  ...
  return new Promise(async function (resolve, reject) {
    const accessToken = await getAccessTokenCashBack();
    const url = getHostEndPointsCashback(config) + EndPointsCashBack.orgSpecific + config.app_id + EndPointsCashBack.promotions;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, null);
...
export async function setConfigCashBack(appConfig) {
  if (appConfig != null && appConfig instanceof AppConfig) {
    ...
    if (client_id != null && client_id !== 'undefined' && client_secret != null && client_secret !== 'undefined' && app_id != null && app_id !== 'undefined' && environment != null && environment !== 'undefined') {
      appConfig = new AppConfig(client_id, client_secret, app_id, environment);
      await EncryptedStorage.setItem('waivpay_sdk_config_cashback_app_id', JSON.stringify(appConfig));
    } else {
      throw new Error('All parameters need to be passed to set config');
    }
  } else {
...
export async function getPromotion(promotionId) {
  ...
  return new Promise(async function (resolve, reject) {
    const accessToken = await getAccessTokenCashBack();
    const url = getHostEndPointsCashback(config) + EndPointsCashBack.orgSpecific + config.app_id + EndPointsCashBack.promotions + '/' + promotionId;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, null);
...
export async function createClaim(claim, promotionId) {
  ...
  return new Promise(async function (resolve, reject) {
    const accessToken = await getAccessTokenCashBack();
    const url = getHostEndPointsCashback(config) + EndPointsCashBack._api + EndPointsCashBack.promotions + '/' + promotionId + EndPointsCashBack.claims;
...
export async function getClaims(external_user_id) {
  ...
  return new Promise(async function (resolve, reject) {
    const accessToken = await getAccessTokenCashBack();
    const url = getHostEndPointsCashback(config) + EndPointsCashBack.api + EndPointsCashBack.claims + '?external_user_id=' + external_user_id;
    const responseText = await sendToEndPoint(config, 'GET', url, accessToken, null);
...
async function getConfigCashBack() {
  let appConfig = new AppConfig();
  const config = await EncryptedStorage.getItem('waivpay_sdk_config_cashback_app_id');
