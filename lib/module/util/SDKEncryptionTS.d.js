export function encryptFromSDK2(val, key) {
  ...
  var PUBLIC_KEY_E = b64tohex("AQAB"); // API Entrypoint for Encryption function, Will use key passed in, or the form's key if not supplied.
  ...
  function getModalUrl(config) {
    ...
    var url = null; // First check for a full URL, and sub 
  ...
  function setButtonProcessed(message) {
    if (window.ewayActivePayNowButton) {
      ...
      createInfoIcon(window.ewayActivePayNowButton, "Your transaction has been accepted for processing. " + message.replace("ID:", "Receipt Number:").replace(/,AC:.*/, "")); // two options Redirect to result page with Access Code in URL if specified) or submit
  ...
  function handleModalMessage(event) {
    ...
    if (event.origin.match(/ewaypayments.com/) || event.origin.match(/localhost:44300/) || event.origin.match(/ewaylabs.cloud$/)) {
      ...
      if (event.data.match(/^LoadOK/i)) {
        ...
        setVar("FrameLoad", "ok");
        removeIFrame("eway-payment-window"); // decode message and fire the call back, then clear the variable
  ...
  function addPayNowButton(tag) {
    ...
    if (endpoint) {
      ...
      button.onclick = function () {
        ...
        setButtonProcessing(); // create iFrame
  ...
  RSAKey.prototype.encrypt = RSAEncrypt; // base64.js
