...
async function sendToEndPoint(
  ...
  consoleLog(
    config,
    'sendToEndPoint TS ' + accessType + ' ' + url + ' ' + accessToken
  );
...
export async function activateBeacon() {
  ...
  consoleLog(config, 'API call - activateBeacon');
...
export async function removeCard(cardId: string): Promise<Card> {
  ...
  return new Promise(async function (resolve, reject) {
    ...
    const url =
      ...
      '/' +
      cardId;
    await sendToEndPoint(config, 'DELETE', url, accessToken, '')
...
export async function searchCards(): Promise<CardList> {
  const config = await getConfig();
  return new Promise(async function (resolve, reject) {
    try {
      const accessToken = await getUserAccessToken();
...
// Delete user profile
export async function deleteProfile(): Promise<Profile> {
  const config = await getConfig();
...
export async function cardCallBack(
  callBackUrl: string,
  token: string
  ...
  return new Promise(async function (resolve, reject) {
    ...
    await sendToEndPoint(config, 'POST', url, accessToken, JSON.stringify(req))
      .then((response) => {
